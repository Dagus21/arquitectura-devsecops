...
// AÑADIDO: Importamos el icono ZoomIn
...
export default function ProductQuickView({ product, isOpen, setIsOpen }: Props) {
  ...
  // NUEVO ESTADO: Controla si la imagen está en pantalla completa
  ...
  const increment = () => {
    if (!isMaxReached) setQuantity((prev) => prev + 1);
  };
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => {
        setIsOpen(open);
        if (!open) setIsZoomed(false); // Reseteamos el zoom si cierran el modal principal
      }}>
        <DialogContent className="w-[95vw] sm:max-w-[700px] p-0 bg-white border-none shadow-2xl rounded-2xl max-h-[90vh] overflow-y-auto sm:overflow-hidden flex flex-col sm:block">
          ...
          <div className="flex flex-col sm:flex-row h-full relative">
            ...
            {/* SECCIÓN DE IMAGEN ACTUALIZADA CON ZOOM */}
            <div 
            ...
            >
              {product.imagenUrl ? (
                <>
                  <img src={product.imagenUrl} alt={product.nombre} className="object-cover w-full h-full absolute inset-0" />
                  
                  {/* Overlay oscuro y lupa que aparece al hacer hover (indicador visual) */}
            ...
            <div className="w-full sm:w-3/5 p-6 sm:p-8 flex flex-col flex-grow">
              <DialogHeader className="mb-4 sm:mb-6 text-left space-y-1 mt-2 sm:mt-0">
                ...
                <DialogDescription className="sr-only">
                  Detalles y compra de {product.nombre}
                </DialogDescription>
                  ...
                  <>
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center border border-gray-300 rounded-lg bg-white overflow-hidden shadow-sm">
                        <button onClick={decrement} className="p-2 hover:bg-gray-100 text-gray-600 active:bg-gray-200 transition-colors">
                        ...
                        <button 
                          ...
                          className={`p-2 transition-colors ${isMaxReached ? 'text-gray-300 bg-gray-50 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-600 active:bg-gray-200'}`}
                        >
      ...
      {/* NUEVA SECCIÓN: OVERLAY PANTALLA COMPLETA PARA LA IMAGEN */}
